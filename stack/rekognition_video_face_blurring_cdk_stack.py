...
class RekognitionVideoFaceBlurringCdkStack(cdk.Stack):
    ...
    def __init__(self, scope: cdk.Construct, construct_id: str, **kwargs) -> None:
        ...
        #Lambda#
        ###############################################################################################
        downloadM3u8Function = lambda_.DockerImageFunction(self, "DownloadM3U8DockerFunction", timeout=cdk.Duration.seconds(600), memory_size=2048,
                                                           code=lambda_.DockerImageCode.from_image_asset("./stack/lambdas/awsdna-download-m3u8"))

        # Adding the S3 output bucket name as an ENV variable to the blurFacesFunction
        downloadM3u8Function.add_environment(
            key="OUTPUT_BUCKET", value=inputImageBucket.bucket_name)

        downloadM3u8Function.add_to_role_policy(_iam.PolicyStatement(
        ...
        startFaceDetectFunction = lambda_.Function(self, "StartFaceDetectFunction", timeout=cdk.Duration.seconds(600), memory_size=512,
                                                   code=lambda_.Code.from_asset(
                                                       './stack/lambdas/rekopoc-start-face-detect'),
        ...
        checkStatusFunction = lambda_.Function(self, "CheckStatusFunction", timeout=cdk.Duration.seconds(600), memory_size=512,
                                               code=lambda_.Code.from_asset(
                                                   './stack/lambdas/rekopoc-check-status'),
                                               handler="lambda_function.lambda_handler",
                                               runtime=lambda_.Runtime.PYTHON_3_7)
        ...
        getTimestampsFunction = lambda_.Function(self, "GetTimestampsFunction", timeout=cdk.Duration.seconds(600), memory_size=512,
                                                 code=lambda_.Code.from_asset(
                                                     './stack/lambdas/rekopoc-get-timestamps-faces'),
                                                 handler="lambda_function.lambda_handler",
                                                 runtime=lambda_.Runtime.PYTHON_3_7)
        ...
        # Adding the S3 output bucket name as an ENV variable to the blurFacesFunction
        blurFacesFunction.add_environment(
        ...
        choice.when(sfn.Condition.string_equals("$.body.job_status",
                    "IN_PROGRESS"), wait_1.next(update_job_status))
        choice.when(sfn.Condition.string_equals("$.body.job_status", "SUCCEEDED"),
